%function [xi yi zi couleur] = Devoir4(poso,xyp,he)
function [xi yi zi couleur pos] = v2(poso,xyp,he)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if he == H_v
    ...
    if poso(1) == 0
      theta_i = atan(H_v/poso(2));
      theta_f = atan((H_p-H_v)/poso(2));
      cote_adj = poso(2);
    elseif poso(2) == 0 && poso(3)>0
      theta_i = atan(H_v/poso(1));
      theta_f = atan((H_p-H_v)/poso(1));
    else
      theta_i = atan((abs(H_v))/poso(1));
      theta_f = atan((H_p-H_v)/poso(1));
    endif
    
  else   
    if poso(1) == 0
      theta_i = atan(he/poso(2));
      theta_f = atan((H_p-he)/poso(2));
      cote_adj = poso(2);
    elseif poso(2) == 0 && poso(3)>0
      theta_i = atan(he/poso(1));
      theta_f = atan((H_p-he)/poso(1));
    else
      theta_i = atan((abs(he))/poso(1));
      theta_f = atan((H_p-he)/poso(1));
    endif    
  ...
  while (theta <= (theta_f+k))
    %fprintf("%d\n", theta);
    pos(end+1,:) = poso(3)+(cote_adj*(tan(theta)));
    z = he+(cote_adj*(tan(theta)));
    

    [rpp, u_ip, v_ip, up] = calculer_rp (poso, R_p, z, rc_p);
